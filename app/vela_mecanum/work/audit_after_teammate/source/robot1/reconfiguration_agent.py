...
def main():
    ...
    # Keep SIGINT under application control so the fail-safe zero commands are
    ...
    def publish_control():
        ...
        # Idle agents used to publish zero velocity and status at 20 Hz.  With
        # four agents and four command bridges, the shared status stream was
        # amplified into roughly 1,600 small UDP packets/s at each bridge.
        # Keep active control comfortably inside the bridge's 0.25 s freshness
        # deadline, while making idle traffic cheap.  A transition to stop is
        # always published on the next 50 ms scheduler tick.
