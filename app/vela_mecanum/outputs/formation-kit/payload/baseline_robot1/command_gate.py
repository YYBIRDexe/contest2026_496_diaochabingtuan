...
class CommandGate:
    def __init__(self, max_speed: float = 0.875, max_yaw: float = 0.30, max_acceleration: float = 2.0):
    ...
    def step(
        ...
        if reason is None:
            self.active = True
            elapsed = 0.05 if not math.isfinite(self.output_time) else max(0.001, min(0.1, now - self.output_time))
            maximum_change = self.max_acceleration * elapsed
            target_x, target_y, target_yaw = self.last_command
            current_x, current_y, _ = self.output
            delta_x, delta_y = target_x - current_x, target_y - current_y
            delta = math.hypot(delta_x, delta_y)
            if delta > maximum_change:
                delta_x *= maximum_change / delta
                delta_y *= maximum_change / delta
            self.output = (current_x + delta_x, current_y + delta_y, target_yaw)
            self.output_time = now
            return self.output, "forwarding"
