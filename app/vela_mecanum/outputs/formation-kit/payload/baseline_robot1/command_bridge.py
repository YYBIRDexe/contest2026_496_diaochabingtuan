...
def motion_limits(path: Path) -> tuple[float, float]:
    data = json.loads(path.read_text(encoding="utf-8"))
    limits = data["limits"]
    max_speed = float(limits["max_linear_mps"])
    max_acceleration = float(limits["max_acceleration_mps2"])
    if not math.isfinite(max_speed) or max_speed <= 0.0:
        raise ValueError("max_linear_mps must be positive and finite")
...
def main() -> None:
    ...
    max_speed, max_acceleration = motion_limits(args.config)
    gate = CommandGate(max_speed=max_speed, max_acceleration=max_acceleration)
