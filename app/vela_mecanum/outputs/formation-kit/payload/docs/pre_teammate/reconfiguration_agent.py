...
def main():
    ...
    for name in ROBOT_IDS:
        subscriptions.append(node.create_subscription(PoseWithCovarianceStamped, f"/{name}/amcl_pose", pose_callback(name), 10))
    subscriptions.append(node.create_subscription(LaserScan, f"/{robot_id}/scan_raw", scan_callback, 10))
