...
def load_grid(pgm_path):
    """Build an OccupancyGrid using the resolution and origin of the map YAML."""
    ...
    return OccupancyGrid(pgm_path, resolution, origin)
...
def main():
    ...
    def on_scan(robot):
        def callback(_message: LaserScan):
            with lock:
    ...
    for robot in ROBOT_IDS:
        ...
        subscriptions.append(node.create_subscription(LaserScan, f"/{robot}/scan_raw", on_scan(robot), qos_profile_sensor_data))
