...
FORMATIONS = ("square", "line", "circle", "diamond", "triangle")
...
class FormationPlanner:
    def __init__(
        ...
        separation_m: float = MIN_SEPARATION_M,
        ...
        self.segment_sample_m = max(0.025, map_grid.resolution if map_grid is not None else 0.025)
    ...
    def plan(self, current: dict[str, Pose], request: Request) -> Plan:
        ...
        for targets, slots, estimate in choices[:12]:
            ...
            if total < best_cost:
                ...
                candidate = Plan(request, targets, slots, steps, _minimum_separation(targets))
                try:
                    self.verify(current, candidate)
    ...
    def _validate(self, current: dict[str, Pose], request: Request) -> None:
        ...
        if not MIN_SEPARATION_M <= request.spacing_m <= 2.5:
            raise PlanningError("spacing must be between 0.30 and 2.5 m")
    ...
    def _segment_free(
        ...
        sample_count = max(1, ceil(distance / self.segment_sample_m))
    ...
    def _astar(self, start: tuple[float, float], goal: tuple[float, float], positions: dict[str, tuple[float, float]], moving: str):
        ...
        world_cache: dict[tuple[int, int], tuple[float, float]] = {}

        def segment_free(first: tuple[float, float], second: tuple[float, float]) -> bool:
            key = (first, second) if first <= second else (second, first)
            cached = segment_cache.get(key)
            if cached is not None:
                return cached
            result = self._segment_free(first, second, positions, moving)
        ...
        def world(cell):
            point = world_cache.get(cell)
            if point is None:
                point = (x0 + cell[0] * step, y0 + cell[1] * step)
                world_cache[cell] = point
            return point

        start_cell, goal_cell = snap(start), snap(goal)
        if segment_free(start, goal):
            return [goal]
        if not segment_free(start, world(start_cell)):
            return None
        ...
        while frontier and visited < 6000:
            ...
            visited += 1
            if segment_free(world(cell), goal):
                path = [goal]
                while cell != start_cell:
                    path.append(world(cell))
                    cell = previous[cell]
                path.reverse()
                return _shortcut(start, path, segment_free)
            for dx, dy in directions:
                neighbor = (cell[0] + dx, cell[1] + dy)
                if not segment_free(world(cell), world(neighbor)):
                    continue
...
def _shortcut(start, path, segment_free):
    result = []
    index = 0
    current = start
    while index < len(path):
        ...
        for candidate in range(len(path) - 1, index - 1, -1):
            if segment_free(current, path[candidate]):
                farthest = candidate
                break
...
def _offsets(formation: str, spacing: float) -> tuple[tuple[float, float], ...]:
    ...
    return {
        ...
        "triangle": ((-triangle_side / 2, -triangle_height / 3), (triangle_side / 2, -triangle_height / 3),
                      (0, 2 * triangle_height / 3), (0, 0)),
    }[formation]
